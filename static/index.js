...
function generateTile() {
  ...
  document.getElementById(`t${randY},${randX}`).textContent = value;
  document.getElementById(`${randY},${randX}`).style.backgroundColor =
    colors[value];
  positions[randY][randX] = value;
  ...
  for (i = 2; i >= 0; i--) {
    for (x = 0; x < 4; x++) {
      for (z = 3 - i; z > 0; z--) {
        ...
        if (
          positions[x][i] != 0 &&
          (positions[x][i + z] == 0 || positions[x][i + z] == positions[x][i])
        ) {
  ...
  for (i = 2; i >= 0; i--) {
    for (x = 0; x < 4; x++) {
      for (z = 3 - i; z > 0; z--) {
        ...
        if (
          positions[i][x] != 0 &&
          (positions[i + z][x] == 0 || positions[i + z][x] == positions[i][x])
        ) {
          for (let t = 1; t < i + z - i; t++) {
...
function checkWinLoss() {
  ...
  for (let i = 0; i < 4; i++) {
    for (let x = 0; x < 4; x++) {
      ...
      if (
        i != 0 &&
        (positions[i - 1][x] == positions[i][x] || positions[i - 1][x] == 0)
      ) {
        return 0;
      }
      if (
      ...
      if (
        x != 0 &&
        (positions[i][x - 1] == positions[i][x] || positions[i][x - 1] == 0)
      ) {
        return 0;
      }
      if (
...
document.addEventListener("keydown", async (e) => {
  if (!disabled) {
    ...
    if (JSON.stringify(copy) != JSON.stringify(positions)) {
